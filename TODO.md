...
| 5 | Add index completeness stats to `dig_status` | P3 | done | PLACEHOLDER | `dig_status` reports connectivity and config but not index health. The agent (and user) can't tell if results are complete or if files were skipped. **Fix:** per-repo section adds: `Indexed: 87/90 files (3 skipped: generated) · Symbols: 342 types, 1,204 methods · Cache age: 2h14m · Commit: abc1234`. Requires reading `meta.json` + `index.dat` stats. Helps the agent trust results ("index is complete") or escalate ("index is stale/incomplete"). **Ref:** claude-os reports health metrics: embedding coverage %, document age, similar pairs, actionable recommendations — all surfaced in the health check endpoint. | `src/tools/digStatus.ts`, `src/cacheManager.ts` |
