...
- **Transitive `<ProjectReference>` expansion.** Both `wildcard` and `explicit` modes call `expandProjectReferences` after initial package resolution: it walks each resolved package's `.csproj`, follows `<ProjectReference Include="...csproj">` tags, and adds the referenced project when (a) it exists as a candidate in the same repo's recursive `discoverPackages` output (sibling-only filter), and (b) for `wildcard` only, it matches the `packageFilter` prefix. Explicit mode has no prefix gate — exact name in `candidateMap` is the only filter. Expansion is idempotent across repeated `ensureReady` calls because the BFS visited-set is rebuilt from the current `repoConfig.packages` each call. Cycles terminate via the visited set.
