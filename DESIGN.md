...
**Output:** For `symbol` mode: markdown listing matching symbols (type name with generics, kind with modifiers, file path — e.g. `**Entity<TId>** (abstract class)`), grouped by `## PackageName` headings in cross-package mode, capped at 100 total matches. Results ranked by relevance score (exact > word-boundary > substring), then by dependency-graph importance (types with more implementors/subclasses within the package rank higher), then alphabetically. For `implements` mode: matching types with their base type list (e.g. `**SqlRepo<T>** (sealed class) : IRepo, IAuditable`), same grouping and cap. For `references` mode: file paths with occurrence counts (e.g. `` `Services/OrderService.cs` (3 occurrences) ``), capped at 50 files. Index cached per package as flat pipe-delimited file (`index.dat`) with optional base-type, generics, modifiers, and refCount fields, invalidated by commit hash. References mode reads source files directly (no cache).
...
- **Package filtering.** A `packageFilter` field on a repo (e.g. `"packageFilter": "MyCompany.*"`) narrows the exposed package list to the intersection of (workspace-referenced packages matching the filter prefix ∩ on-disk `.csproj` directories). The repo `name` is a plain identifier (e.g. `"MyCompany.NuGet"`); `packages` (explicit list) and `packageFilter` are mutually exclusive. References are collected by a recursive workspace scan over `.sln`, `.slnx`, `Directory.Packages.props`, `Directory.Build.props`, and `Directory.Build.targets` files, skipping `.git`/`.digger`/`node_modules`/`bin`/`obj`/`.vs`/`.idea`/`packages`. The scan runs once per `ensureAllReady()` call and writes its full result to `<cacheDir>/solution-scan.json`. When a filtered repo resolves to zero packages, the per-repo `error` is surfaced by each tool with a recommendation to switch to an explicit `packages` list.
- **Branch tracking.** An optional `branch` field on a repo (e.g. `"branch": "develop"`) controls which git branch is cloned and fetched for managed repos. When set, `clone` uses `-b <branch>` and `fetch` uses `<branch>` as the refspec; when omitted, the refspec falls back to `HEAD` so `FETCH_HEAD` always resolves to a single deterministic commit. Only meaningful for managed clones (Mode A) — for local repos (Mode B), the user controls the branch themselves. Branch names are validated (no `..`, no leading `-`, no `//`, safe charset).
- **Fetch resets working tree.** After every successful `git fetch` on a managed clone, `gitClient.fetch` runs `git reset --hard FETCH_HEAD`. This keeps three views in sync: `revParse(HEAD)` (cache-invalidation hash), `git show HEAD:<path>` (used by `dig_file`/`readFile`), and `fs.readdir` on the working tree (used by `discoverPackages`). Without the reset, fetch would update only `FETCH_HEAD` and silent divergence would hide new upstream content from disk-walking tools. Managed clones live in detached-HEAD state after the first fetch — acceptable since no code path resolves the local branch ref. Mode B local repos skip this code path entirely (user owns the working tree).
- **Recursive csproj discovery.** `discoverPackages` walks recursively under `${repoPath}/${sourceRoot}/` looking for directories containing `{dirName}/{dirName}.csproj`. Skips `IGNORED_DIRS` (`.git`, `.digger`, `node_modules`, `bin`, `obj`, `.vs`, `.idea`, `packages`), test projects (`.Tests`, `.Specs`, `.Benchmarks`, `.IntegrationTests`), and symlinks. Grouping containers with non-package-shaped names (e.g. `Common Libraries`) are not registered as packages but are still descended into so nested packages inside them are reachable. Sorts entries alphabetically for deterministic walk order. Dedupes by package name preferring the shallowest depth — shadow projects in deeper subtrees do not override real projects at the canonical depth. Capped at `MAX_DISCOVERED_PACKAGES` (200).
- **Transitive `<ProjectReference>` / `<PackageReference>` expansion.** Both `wildcard` and `explicit` modes call `expandProjectReferences` after initial package resolution: it walks each resolved package's `.csproj`, follows `<ProjectReference Include="...csproj">` (path-based) **and** CPM-style `<PackageReference Include="...">` tags (refs without an inline `Version` or `VersionOverride` — those signal centralised versioning via `Directory.Packages.props`; versioned refs are treated as published-NuGet pins and skipped). Adds the referenced project when (a) it exists as a candidate in the same repo's recursive `discoverPackages` output (sibling-only filter), and (b) for `wildcard` only, it matches the `packageFilter` prefix. Explicit mode has no prefix gate — exact name in `candidateMap` is the only filter. The sibling-only filter is what makes harvesting `<PackageReference>` safe: externally-published NuGet packages are never registered as candidates, so they are silently ignored. Expansion is idempotent across repeated `ensureReady` calls because the BFS visited-set is rebuilt from the current `repoConfig.packages` each call. Cycles terminate via the visited set.
